...
## Analisando Commits com `git log`:
    * Permite visualizar o histórico de commits, facilitando o acompanhamento das mudanças no projeto.
    ...
    * `git log --pretty` ou `git log --format`: Personalização da exibição.
...
## Analisando Detalhes de Commits com `git show`:
    ...
    * `git log --oneline`: Lista commits para identificar um commit específico.
    * `git show {hash do commit}`: Detalhes do commit (autor, data, mensagem, diff).
    * "Q" para sair da visualização.
...
## Analisando Mudanças com `git status` e `git diff`:
    ...
    * Modificações em arquivos: `git status` mostra mudanças não staged.
    ...
    * `git log --oneline`: Visualiza o histórico de commits.
    * Introdução ao conceito de branches.